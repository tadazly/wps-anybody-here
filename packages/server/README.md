...
- `POST /api/webhooks/repo-push`：表格仓库 push webhook，按变更文件推送到已打开对应表格的客户端。
- `POST /webhooks/repo-push`：同上，给 webhook 配置时使用更短路径。
- `GET /health`：健康检查。
...
```txt
http://<发布机>:18080/api/webhooks/repo-push
```

服务端会从常见 GitHub、GitLab、Gitee push payload 中读取仓库地址、提交者、commit 内容和变更文件列表。只有当变更文件路径匹配已打开表格的仓库相对路径时，客户端才会在协作面板顶部显示通知。

也可以用通用 JSON 直接测试：

```json
{
  "repoUrl": "https://example.com/team/table-config",
  "workbookPath": "tables/config.xlsx",
  "pusherName": "配置同学",
  "message": "更新角色数值表",
  "commitId": "abc1234"
}
...
如果仓库平台 payload 里的仓库地址和插件设置里的“表格仓库地址”不完全一致，可以在 webhook URL 上补 `repoUrl` 查询参数：

```txt
http://<发布机>:18080/api/webhooks/repo-push?repoUrl=https%3A%2F%2Fexample.com%2Fteam%2Ftable-config
```
